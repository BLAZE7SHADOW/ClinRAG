...
- [x] **Fri (combined Thu+Fri)** — Embeddings + vector store baseline (Cohere v4 via Bedrock, FAISS), hybrid retrieval (BM25 + reciprocal rank fusion with dense), and LLM generation (Claude Haiku 4.5 via Bedrock) all working end-to-end. See Notes.md Day 3/4. Plan slipped a day; Sat/Sun shift back accordingly.
...
**Reranking decision:** not built today. Decide after Saturday's RAGAS numbers are in — add it only if the measured results show a real gap it would close, not assumed upfront.
